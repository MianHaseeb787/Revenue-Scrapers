...
sheet_id = "1j61J05JzsTj_x5R8u9V3yDz_JuhRCMNQG-ZbavlUzpY"
...
class silvermapleSpider(scrapy.Spider):
    ...
    start_urls = ['https://voixceleste.cc/']
    ...
    def parse(self, response):
            ...
            products = ['https://voixceleste.cc/p/estradiol-enanthate-400/',
                        'https://voixceleste.cc/p/estradiol-enanthate-500/',
                        'https://voixceleste.cc/p/estradiol-undecylate-500/'
                        ]

            for index, product in enumerate(products):
                prices = ["35", "40", "50"]
                meta = {'index': index, 'price' : prices[index]}  # Include the index in metadata
                yield scrapy.Request(url=product, callback=self.parseVoixProduct, meta=meta)

          

    def parseVoixProduct(self, response):
        price = response.meta.get('price')
        productName = response.css('.wp-block-post-title::text').get().strip()
        productPrice = price
